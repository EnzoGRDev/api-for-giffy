...
favoritesRouter.post('/', authExtractor, async (req, res) => {
  ...
  if (!favorite) {
    ...
    const isUserInFav = favorite ? await favorite.users.toString().includes(user._id.toString()) : null;
  ...
  let newsFavoritesOfUser = await User.find({
    username
  }).populate("favorites");
...
favoritesRouter.delete('/:favId', authExtractor, async (req, res) => {
  ...
  user = user[0];
  favorite = favorite[0];
  if (!favorite) return res.status(404).json({
    error: "no existe el favorito"
  });
